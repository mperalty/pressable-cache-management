...
$pcm_microcache_dir = plugin_dir_path( __FILE__ );

require_once $pcm_microcache_dir . 'interface-pcm-microcache-backend.php';
require_once $pcm_microcache_dir . 'class-pcm-microcache-filesystem-backend.php';
require_once $pcm_microcache_dir . 'class-pcm-microcache-tableindex-backend.php';
...
function pcm_microcache_delete_artifact( string $path ): void {
...
/**
 * Log durable microcache operational failures.
 *
 * @param string $message Message to log.
 *
 * @return void
 */
function pcm_microcache_log_message( string $message ): void {
	if ( '' === $message ) {
		return;
	}

	// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Microcache storage failures are logged for cache integrity diagnostics.
	error_log( $message );
}
...
function pcm_microcache_record_stat( string $event, string $key, float $build_ms, string $builder_name ): void {
	...
	// Use cache key as the cost identifier — it's far more descriptive than
	// "closure_builder" when every builder is an anonymous function.
	// Normalize dynamic suffixes (hashes, post IDs) to group related entries.
	$cost_key = pcm_microcache_normalize_cost_key( $key, $builder_name );

	if ( ! isset( $stats['builder_costs'][ $cost_key ] ) ) {
		$stats['builder_costs'][ $cost_key ] = array(
			'calls'    => 0,
			'total_ms' => 0,
			'hits'     => 0,
			'misses'   => 0,
	...
	++$stats['builder_costs'][ $cost_key ]['calls'];
	$stats['builder_costs'][ $cost_key ]['total_ms'] += $build_ms;

	if ( 'hit' === $event || 'stale' === $event ) {
		++$stats['builder_costs'][ $cost_key ]['hits'];
	} else {
		++$stats['builder_costs'][ $cost_key ]['misses'];
	}
...
/**
 * Normalize a cache key into a human-readable cost-tracking label.
 *
 * Dynamic suffixes (MD5 hashes, numeric post IDs) are collapsed so that
 * related entries group together, e.g. "related_posts_42_4" → "related_posts".
 *
 * @param string $key          Raw cache key.
 * @param string $builder_name Resolved callable name (fallback).
 *
 * @return string
 */
function pcm_microcache_normalize_cost_key( string $key, string $builder_name ): string {
	if ( '' === $key ) {
		return $builder_name;
	}

	// Strip trailing MD5 hashes (32 hex chars) and numeric IDs.
	$normalized = preg_replace( '/_[a-f0-9]{32}$/i', '', $key );
	$normalized = preg_replace( '/_\d+(_\d+)*$/', '', $normalized );
		...
		static function () use ( $post_id, $limit ): array {
			...
			foreach ( (array) $query->posts as $related ) {
				$cards[] = array(
					...
					'thumbnail' => get_the_post_thumbnail_url( $related, 'medium' ) ? get_the_post_thumbnail_url( $related, 'medium' ) : '',
				);
			}
		...
		static function () use ( $product_id, $limit ): array {
			...
			foreach ( $related_ids as $rid ) {
				...
				$cards[] = array(
					...
					'thumbnail'      => get_the_post_thumbnail_url( $rid, 'woocommerce_thumbnail' ) ? get_the_post_thumbnail_url( $rid, 'woocommerce_thumbnail' ) : '',
	...
	return pcm_microcache_get_or_build(
		...
		static function () use ( $product_id ): array {
			...
			foreach ( $all_ids as $index => $attachment_id ) {
				...
				$images[] = array(
					'id'          => $attachment_id,
					'is_featured' => ( 0 === $index && $attachment_id === $featured ),
					'full'        => esc_url( $full ),
					'large'       => esc_url( $large ? $large : $full ),
					'thumbnail'   => esc_url( $thumb ? $thumb : $full ),
...
function pcm_microcache_get_health_summary(): array {
	...
	foreach ( (array) $stats['builder_costs'] as $name => $row ) {
		...
		$misses = isset( $row['misses'] ) ? absint( $row['misses'] ) : 0;
		...
		<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
			<div>
				...
				<p style="margin:0 0 8px;font-size:11px;color:#6b7280;">
					<?php echo esc_html__( 'A "builder" is the callback that generates data when no cached copy exists (cache miss) or when an existing copy expires. Each row represents a cached data source, sorted by total build time. A high cache-hit ratio means the microcache is serving stored results instead of rebuilding.', 'pressable_cache_management' ); ?>
				</p>
				...
				<?php else : ?>
					<ul style="margin:0;padding-left:18px;font-size:12px;">
						<?php foreach ( $summary['top_builders'] as $row ) : ?>
							<?php
							$hit_ratio = $row['calls'] > 0
								? round( ( $row['hits'] / $row['calls'] ) * 100 )
								: 0;

							if ( $row['hits'] > 0 && $hit_ratio >= 50 ) {
								$status_label = __( 'Cached', 'pressable_cache_management' );
								$status_color = '#059669';
								$status_icon  = "\xe2\x9c\x93"; // checkmark
							} elseif ( $row['hits'] > 0 ) {
								$status_label = __( 'Low hit rate', 'pressable_cache_management' );
								$status_color = '#d97706';
								$status_icon  = "\xe2\x97\x8b"; // circle
							} else {
								$status_label = __( 'Building only', 'pressable_cache_management' );
								$status_color = '#6b7280';
								$status_icon  = "\xe2\x80\x94"; // dash
							}
							?>
							<li style="margin-bottom:6px;">
								<strong><?php echo esc_html( $row['builder'] ); ?></strong>
								<span style="color:<?php echo esc_attr( $status_color ); ?>;font-size:11px;margin-left:4px;"
									title="<?php echo esc_attr( $hit_ratio . '% cache-hit ratio' ); ?>">
									<?php echo esc_html( $status_icon . ' ' . $status_label ); ?>
								</span>
								<br>
								<span style="color:#374151;">
									<?php echo esc_html( (string) $row['total_ms'] ); ?>ms total
									(<?php echo esc_html( (string) $row['calls'] ); ?> lookups, avg <?php echo esc_html( (string) $row['avg_ms'] ); ?>ms build)
								</span>
								<br><span style="color:#6b7280;">
									<?php echo esc_html( (string) $row['hits'] ); ?> cache hits / <?php echo esc_html( (string) $row['misses'] ); ?> rebuilds
									— <?php echo esc_html( $hit_ratio . '%' ); ?> served from microcache
								</span>
							</li>
						<?php endforeach; ?>
					</ul>
