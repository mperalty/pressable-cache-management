...
function pcm_scenario_build_variants( array $config ): array {
    ...
    $temps = array();
    if ( $warm ) {
        $temps[] = array( 'id' => 'warm', 'label' => 'Warm' );
    }
    if ( $cold ) {
        $temps[] = array( 'id' => 'cold', 'label' => 'Cold' );
    }
    if ( empty( $temps ) ) {
        $temps[] = array( 'id' => 'warm', 'label' => 'Warm' );
    }
    ...
    $devices = array();
    if ( $desktop ) {
        $devices[] = array( 'id' => 'desktop', 'label' => 'Desktop', 'ua' => PCM_SCENARIO_UA_DESKTOP );
    }
    if ( $mobile ) {
        $devices[] = array( 'id' => 'mobile', 'label' => 'Mobile', 'ua' => PCM_SCENARIO_UA_MOBILE );
    }
    if ( empty( $devices ) ) {
        $devices[] = array( 'id' => 'desktop', 'label' => 'Desktop', 'ua' => PCM_SCENARIO_UA_DESKTOP );
    }
    ...
    $cookie_modes = array();
    if ( $no_cookie ) {
        $cookie_modes[] = array( 'id' => 'no_cookie', 'label' => 'No Cookie' );
    }
    if ( $cookie ) {
        $cookie_modes[] = array( 'id' => 'cookie', 'label' => 'With Cookie' );
    }
...
function pcm_scenario_probe_variant( string $url, array $variant ): array {
    ...
    $raw_headers = ( $raw_headers instanceof \WpOrg\Requests\Utility\CaseInsensitiveDictionary || $raw_headers instanceof \Requests_Utility_CaseInsensitiveDictionary )
        ? $raw_headers->getAll()
        : (array) $raw_headers;
...
function pcm_ajax_scenario_scan_resolve_urls(): void {
    ...
    wp_send_json_success( array( 'urls' => $urls, 'source' => $source ) );
}
...
function pcm_ajax_scenario_scan_run(): void {
    ...
    $variant_config = isset( $_POST['variants'] ) ? json_decode( wp_unslash( (string) $_POST['variants'] ), true ) : array();
