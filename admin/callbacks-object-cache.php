...
function pressable_cache_management_callback_section_cache(): void {
	...
	$branding_disabled                     = is_array( $remove_pressable_branding_tab_options )
		&& isset( $remove_pressable_branding_tab_options['branding_on_off_radio_button'] )
		&& 'disable' === $remove_pressable_branding_tab_options['branding_on_off_radio_button'];

	if ( ! $branding_disabled ) {
		echo '<div><img width="230" height="50" class="pressablecmlogo" src="' . esc_url( plugin_dir_url( __FILE__ ) . 'assets/img/pressable-logo-primary.svg' ) . '" alt="' . esc_attr__( 'Pressable logo', 'pressable_cache_management' ) . '" /></div>';
	}
...
function pressable_cache_management_callback_field_button( $args ): void {
	$id    = $args['id'] ?? '';
	$label = $args['label'] ?? '';
	$field = 'pressable_cache_management_options_' . $id;

	echo '</form>';

	echo '<form method="post" id="flush_object_cache_nonce">';
	echo '<span id="flush_cache_button">';
	echo '<input id="' . esc_attr( $field ) . '" name="pressable_cache_management_options[' . esc_attr( $id ) . ']" type="submit" size="40" value="' . esc_attr__( 'Flush Cache', 'pressable_cache_management' ) . '" class="pcm-btn-primary pcm-btn-block"/>';
	echo '<input type="hidden" name="flush_object_cache_nonce" value="' . esc_attr( wp_create_nonce( 'flush_object_cache_nonce' ) ) . '" />';
	echo '<br/><label class="rad-text" for="' . esc_attr( $field ) . '">' . esc_html( $label ) . '</label>';
	echo '</span>';
	echo '</form>';
