...
if ( null !== filter_input( INPUT_POST, 'purge_edge_cache_nonce', FILTER_UNSAFE_RAW ) ) {
	...
	if ( ! function_exists( 'pcm_pressable_edge_cache_purge_local' ) ) {
		...
		function pcm_pressable_edge_cache_purge_local(): void {
			...
			// 5. Purge domain cache via canonical service
			$result = pcm_cache_service()->purge_edge_cache_domain( 'dashboard-auto-purge' );
