...
if ( ! function_exists( 'pressable_edge_cache_notice_success_enable' ) ) {
	function pressable_edge_cache_notice_success_enable(): void {
		...
		echo '<div class="pcm-branded-notice-body">' . wp_kses_post( $html ) . '</div>';
...
function pcm_ajax_toggle_edge_cache(): void {
	...
	$desired = filter_input( INPUT_POST, 'desired_state', FILTER_UNSAFE_RAW );
	$desired = sanitize_text_field( is_string( $desired ) ? $desired : '' );
