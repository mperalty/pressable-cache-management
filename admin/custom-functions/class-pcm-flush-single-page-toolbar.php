 ...
 * Sourced from official repo flush-single-page-toolbar.php with branded notices.
 */
...
class PCM_Flush_Single_Page_Toolbar {

	public static function bootstrap(): void {
		$options = pcm_get_options();

		if ( empty( $options['flush_object_cache_for_single_page'] ) ) {
			return;
		}
	...
	public static function register(): void {
		...
		if ( is_admin() ) {
			add_action( 'admin_enqueue_scripts', array( self::class, 'load_frontend_toolbar_css' ) );
			return;
		}

		if ( is_admin_bar_showing() ) {
			add_action( 'wp_enqueue_scripts', array( self::class, 'load_frontend_toolbar_css' ) );
		}
	}

	public function add(): void {
		if ( is_admin() ) {
			add_action( 'wp_before_admin_bar_render', array( $this, 'render_admin_toolbar_placeholder' ) );
	...
	public static function load_frontend_toolbar_css(): void {
		$css_path = dirname( dirname( plugin_dir_path( __FILE__ ) ) ) . '/public/css/pcm-toolbar.css';
		wp_enqueue_style(
			'pressable-cache-management-toolbar',
			plugins_url( 'public/css/pcm-toolbar.css', dirname( __DIR__ ) . '/pressable-cache-management.php' ),
			array(),
	...
	public function pcm_delete_current_page_cache(): void {
		...
		$path = esc_url_raw( urldecode( is_string( $path ) ? $path : '' ) );
		...
		if ( ! pcm_cache_service()->flush_batcache_url( $url, 'toolbar-single-page' ) ) {
	...
	public function pcm_purge_current_page_edge_cache(): void {
		...
		if ( pcm_cache_service()->purge_edge_cache_url( $url, 'toolbar-single-page' ) ) {
	...
	public function load_toolbar_css(): void {
		$css_path = plugin_dir_path( __DIR__ ) . 'public/css/toolbar.css';
		wp_enqueue_style(
			'pressable-cache-management-toolbar',
	...
	public function load_toolbar_js(): void {
		$js_path = plugin_dir_path( __DIR__ ) . 'public/js/toolbar.js';
		wp_enqueue_script(
			'pcm-toolbar',
	...
	public function load_remove_branding_toolbar_js(): void {
		$js_path = plugin_dir_path( __DIR__ ) . 'public/js/toolbar_remove_branding.js';
		wp_enqueue_script(
			'pcm-toolbar-branding',
	...
	public function pcm_toolbar_for_page_preview(): void {
		...
		if ( $branding_disabled ) {
			...
			// Combined item: JS fires both Batcache and Edge Cache flushes in sequence.
	...
	// Admin-side toolbar rendering is handled by cache-purge-admin-bar.php.
	// This method exists only as a hook target registered in add() above.
	public function render_admin_toolbar_placeholder(): void {}
}

PCM_Flush_Single_Page_Toolbar::bootstrap();
