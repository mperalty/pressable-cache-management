...
if ( ! function_exists( 'pcm_ajax_flush_object_cache' ) ) {
    function pcm_ajax_flush_object_cache() {
        $nonce = isset( $_POST['flush_object_cache_nonce'] )
            ? sanitize_text_field( wp_unslash( $_POST['flush_object_cache_nonce'] ) )
            : '';

        if ( ! wp_verify_nonce( $nonce, 'flush_object_cache_nonce' ) || ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( array( 'message' => __( 'Unauthorized request.', 'pressable_cache_management' ) ), 403 );
        }
        ...
        wp_send_json_success( array(
            ...
            'timestamp' => (string) get_option( 'flush-obj-cache-time-stamp', '' ),
        ) );
    }
}
...
if ( isset( $_POST['flush_object_cache_nonce'] ) ) {
    $nonce = sanitize_text_field( wp_unslash( $_POST['flush_object_cache_nonce'] ) );

    if ( wp_verify_nonce( $nonce, 'flush_object_cache_nonce' ) && current_user_can( 'manage_options' ) && pcm_flush_all_caches() ) {
